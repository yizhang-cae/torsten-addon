...
%.torsten.hpp: %_torsten__.stan %_torsten_functions__.stan $(MODEL_HEADER) bin/stanc$(EXE)
	...
	@perl -0777 -pi -e 's/([\S]+ *torsten)_(generalOdeModel_rk45)_([^,]+)__\(/$$1::$$2\($$3_functor__(), /gm' $@
	@perl -0777 -pi -e 's/([\S]+ *torsten)_(generalOdeModel_bdf)_([^,]+)__\(/$$1::$$2\($$3_functor__(), /gm' $@
	@perl -0777 -pi -e 's/(#include <stan\/model\/model_header\.hpp>)/$$1\n\n\/\/ Code generated by Torsten version 0.84\n#include <torsten\/torsten\.hpp>\n/s' $@

%.torsten$(EXE) : %.torsten.hpp %_torsten__.stan $(LIBSUNDIALS)
	@echo ''
	@echo '--- Torsten: Linking C++ model ---'
	@test -f $(dir $<)USER_HEADER.hpp || touch $(dir $<)USER_HEADER.hpp
	$(LINK.cc) -O$O $(OUTPUT_OPTION) $(CMDSTAN_MAIN) -I$(TORSTEN_PATH)/include -include $< -include $(dir $<)USER_HEADER.hpp $(LIBCVODES)
